...
flexible command line mail client used to send email messages,
via SMTP (Simple Mail Transfer Protocol) to a mail server.
This is great for testing software that handles email,
or for use by any admin scripts to send log notifications.

0.1, 02/10/2008, john@masinter.net, simple email test
0.2,  add send file to smtp svr, 'er, John Masinter, 02/10/2008
0.3,  simple sent file to smtp mx'er, John Masinter, 02/10/2008
0.4,  simple sent file to smtp mx'er, John Masinter, 02/10/2008
0.5,  simple sent file to smtp mx'er, John Masinter, 02/10/2008
0.6,  simple sent file to smtp mx'er, John Masinter, 02/10/2008
0.7,  simple sent file to smtp mx'er, John Masinter, 02/10/2008
0.8,  simple sent file to smtp mx'er, John Masinter, 02/10/2008
0.9,  simple sent file to smtp mx'er, John Masinter, 02/10/2008
0.10, simple sent file to smtp mx'er, John Masinter, 02/10/2008
0.11, simple sent file to smtp mx'er, John Masinter, 02/10/2008
0.12, simple sent file to smtp mx'er, John Masinter, 02/10/2008
0.13, simple sent file to smtp mx'er, John Masinter, 02/10/2008
0.14, simple sent file to smtp mx'er, John Masinter, 02/10/2008
0.15, simple sent file to smtp mx'er, John Masinter, 02/10/2008
"""
...
RFC2487 OPTIONS:
tls        issue STARTTLS cmd after ehlo and switch to TLS.
           If using "hostname:465" then tls starts automatically.
...
def parse_params():
    ...
    ARG = { "HOST":None, "PORT":SMTP_PORT, 
            ...
            "BIND":[], "BINDCNT":0, "EXIP":None, "TID":False, "TLS":False
    }
    ...
    if verdict==ARG_GOOD:
        for i in range (1, argc):
            ...
            elif arg[0] == "tls":       ARG['TLS'] = True
            elif arg[0] == "notls":     ARG['TLS'] = False
...
def read_file(ARG):
    ...
    if verdict == ARG_GOOD and ARG['FILE'][-1:] == "/":
        # read list of files
        for item in os.listdir(ARG['FILE']):
            if os.path.isfile(ARG['FILE']+item):
                ARG['FILELIST'].append(ARG['FILE']+item)
...
def connect(ARG):
    ...
    s.timeout(10)                              # shorter timeout for connecting only.
    ...
    try:
        ...
        s.timeout(ARG['TIMEOUT'])              # timeout for reading from server
    except socket.error:
        end = int(time.time())
        if (end-beg > 4): print "Server is not answering. It may be powered off."
        else:             print "Server refused connection. Mail daemon may not be running."
        sys.exit(1)
    except:
        print ExceptionInfo()
        sys.exit(1)

    if ARG['PORT']==SECURE_PORT:           # tls?
        do_tls(s,ARG)
...
def do_tls(s,ARG):
    ...
    try:
        s.ssl()
        if ARG['ECHO']:
            print "Success: StartTLS Succeeded.\nServer: %s\nIssuer: %s" % (s._ssl.server(), s._ssl.issuer())
    except socket.error, msg:
        if ARG['ECHO']:
            print "Error: StartTLS Failed: %s" % msg

#--------------------------------------------------------------------------------

def send_tls(s,ARG):
    """send tls if needed per http://www.rfc-editor.org/rfc/rfc2487.txt"""
    if not ARG['TLS']:
        return
    ...
    if ret:
        (cod, txt) = ret
        if cod == 220:
            if ARG['ECHO']: print "Starting TLS..."
            do_tls(s,ARG)
        else:
            if ARG['ECHO']:
                print "Error: Bad return code from starttls."
    else:
        if ARG['ECHO']:
            print "Error: No return code from starttls."
...
def mailone(ARG):
    ...
    if ARG['ECHO']:
        ...
        if ARG['TIME']:
            write_log ("stup:", ARG)
    try:
        s = connect(ARG)                # connect to server
        s.recvsmtp()                    # receive smtp welcome banner
        send_helo(s,ARG)                # send ehlo or helo
        send_tls(s,ARG)                 # starttls if desired
        send_headers(s,ARG)             # send 821 hdrs (mail from, etc)
...
def wait_threads(ARG):
    ...
    # all done
    global g_msgcnt,g_msggood,g_msgbad,g_timbeg # global message count shared by all threads
    if ARG['DOT']: print ""
    timend=time.time()
    timtot=int(timend-g_timbeg)
    if timtot < 1: timtot=1
    rate=""
    if ARG['COUNT'] >= 100:
        msgsec=g_msgcnt/timtot
        msghr=int(msgsec*3600)
        rate="msg/sec: %3.1f  msg/hr: %s" % (msgsec, sockit.commify(msghr))
    if ARG['COUNT'] > 1:
        print >> sys.stderr, "Proc %s Done. Msgs: %s  Good: %s  Bad: %s  Time: %s  %s" % \
        (os.getpid(), sockit.commify("%s"%g_msgcnt), sockit.commify("%s"%g_msggood), sockit.commify("%s"%g_msgbad), \
         sockit.timestr(timtot), rate)
...
def wait_procs(ARG):
    ...
    if num > 1:
        for each in xrange(1, num):
            if ARG['DEBUG']:
                print "WAIT: parent pid %s waiting on child proc: %s" % (os.getpid(),ARG['PIDLST'][each])
...
def main(argv=None):
    ...
    global g_timbeg
    g_timbeg=time.time()    # time we started
    ...
    global g_msggood,g_msgbad
    if g_msggood > g_msgbad: return 0
    else:                    return 1
